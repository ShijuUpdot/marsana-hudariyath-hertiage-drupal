...
/**
 * Provides a form for reverting a Community post revision.
 *
 * @ingroup community_post
 */
class CommunityPostRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'community_post_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Community post: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Community post %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\community_post\Entity\CommunityPostInterface $revision
   ...
   */
  protected function prepareRevertedRevision(CommunityPostInterface $revision, FormStateInterface $form_state) {
