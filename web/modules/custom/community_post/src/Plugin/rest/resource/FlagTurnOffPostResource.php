 ...
 *
 * @RestResource(
 *   id = "flag_turn_off_post_resource",
 *   label = @Translation("Turn off post"),
 *   uri_paths = {
 *     "https://www.drupal.org/link-relations/create" =
 *   "/json-api/{version}/{lang}/flag-turn-off"
 *   }
 * )
 */
class FlagTurnOffPostResource extends ResourceBase {
  ...
  public function post(Request $request, $version, $lang) {
    ...
    if ('community_post' == $data['entity_type']) {
      ...
      $flag_id = 'turn_off_post_notification';
    }

    if ('node' == $data['entity_type']) {
      $entity = Node::load(intval($data['post_id']));
      $flag_id = 'turn_off_node_notification';
    }
