 ...
 *
 * This repository is a service named 'users_access_token.repository'. You can see
 * how the service is defined in users_access_token/users_access_token.services.yml.
 *
 ...
 *
 * @ingroup users_access_token
 */
class MarvistaAPIRepository {
  ...
  public function insert(array $entry) {
    try {
      $return_value = $this->connection->insert('users_access_token')
        ->fields($entry)
        ->execute();
    }
    catch (\Exception $e) {
  ...
  public function update(array $entry) {
    try {
      ...
      $count = $this->connection->update('users_access_token')
        ->fields($entry)
  ...
  public function delete(array $entry) {
    $this->connection->delete('users_access_token')
      ->condition('uid', $entry['uid'])
      ->execute();
  }
   ...
   *
   * @code
   *   // SELECT * FROM {users_access_token} WHERE uid = 0 AND name = 'John'
   *   \Drupal::database()->query(
   *     "SELECT * FROM {users_access_token} WHERE uid = :uid and name = :name",
   *     [':uid' => 0, ':name' => 'John']
   *   )->execute();
   * @endcode
   *
   ...
   * @code
   *   // SELECT * FROM {users_access_token} WHERE uid = 0 AND name = 'John'
   *   \Drupal::database()->select('users_access_token')
   ...
   * altered:
   * @code
   *   // SELECT * FROM {users_access_token} WHERE age > 18
   *   \Drupal::database()->select('users_access_token')
   *     ->fields('users_access_token')
   *     ->condition('age', 18, '>')
   *     ->execute();
  ...
  public function load(array $entry = []) {
    // Read all the fields from the users_access_token table.
    $select = $this->connection
      ->select('users_access_token')
  ...
  /**
   * Load users_access_token records joined with user records.
   *
  ...
  public function advancedLoad() {
    // Get a select query for our users_access_token table. We supply an alias of e
    // (for 'example').
    $select = $this->connection->select('users_access_token', 'e');
